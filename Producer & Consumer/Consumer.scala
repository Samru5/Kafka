...
object Consumer {
  ...
  def main(args: Array[String]) {
    ...
    val groupId = args(2) + "_" + System.currentTimeMillis()
    //val numThreads = Try(args(3).toInt).getOrElse(2)
    val streamingDuration = args(3).toInt
    val databaseServerIp = args(4)
  ...
  def consumeFromUet(zkQuorum: String, topics: String,groupId:String, streamingDuration: Int,
    ...
    val conf = returnConf(false)		//false because running on localhost
